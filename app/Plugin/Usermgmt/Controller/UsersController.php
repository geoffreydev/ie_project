...
class UsersController extends UserMgmtAppController {
	 ...
	 * @var array
	 */
	public $uses = array('Usermgmt.User', 'Usermgmt.UserGroup', 'Usermgmt.LoginToken','Event','Dog');
	/**
			...
			if ($this->request -> isPost()) {
				...
				if ($this->User->RegisterValidate()) {
					...
					if (isset($this->request->data['User']['email_verified']) && $this->request->data['User']['email_verified']) {
						$this->UserAuth->login($user);
                        $this->Session->setFlash(__('Welcome to Canine Clip Joint.  You are now logged in!'));
		...
		if ($this->request -> isPost()) {
			...
			if ($this->User->RegisterValidate()) {
                ...
                if ($this->Session->read('UserAuth.User.user_group_id') == 2){
                    ...
                    $this->redirect('/admins');
                } else if ($this->Session->read('UserAuth.User.user_group_id') == 4) {
                    $this->redirect('/staffs');
                }
	...
	public function editNormalUser($userId=null) {
		if (!empty($userId)) {
			$userGroups=$this->UserGroup->getGroups();
			$this->set('userGroups', $userGroups);
			if ($this->request -> isPut()||$this->request->is('post')) {
				$this->User->set($this->data);
				if ($this->User->RegisterValidate()) {
					$this->User->save($this->request->data,false);
					$this->Session->setFlash(__('The user is successfully updated'));
